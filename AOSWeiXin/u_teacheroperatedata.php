<?php
    ...
    class U_TeacherInfo
    {
        ...
        public function U_TeacherInfo()
        {
            $this->stuRegData = new StudentRegister();
            $this->stuInfoData = new StudentInfo();
			$this->stuLeaveData = new StudentLeave();
			$this->stuScoreData = new StudentScoreInfo();
        }
        
        //插入数据
        public function insterTeacherInfo($openID, $u_name, $name, $className, $tel, $Email, $sex, $weixinNum, $image="http://")
        {
            //            if ($this->checkTeacherInfo($openID))
            //                return false;
            //!院校名!教师姓名!风云班级名字!手机号!邮箱!性别!微信号或者QQ号!
            if (!linkMysql())
                return false;
            
            $tableName = "u_teacherinfo";
            
            $newSex = judgeSex($sex);
            
            $result = mysql_query("INSERT INTO $tableName (openID, u_name, name, className, tel, Email, sex, weixinNum, image) VALUES (\"$openID\",\"$u_name\", \"$name\", \"$className\", \"$tel\", \"$Email\", $newSex, \"$weixinNum\", \"$image\")");
        ...
        public function checkTeacherInfo($openID)
        {
            ...
            $sqlSel="SELECT u_name, name, className, tel, Email, sex, weixinNum, image FROM $tableName WHERE openID='" . $openID . "'";
        ...
        public function getTeacherInfo($openID)
        {
            ...
            $sqlSel="SELECT u_name, name, className, tel, Email, sex, weixinNum,type,image FROM $tableName WHERE openID='" . $openID . "'";
        ...
        public function getTeacherInfoByField($field, $info)
        {
            ...
            $sqlSel="SELECT u_name, name, className, tel, Email, sex, weixinNum,type,image FROM $tableName WHERE $field='" . $info . "'";
        ...
        public function getPrivilegedUserByFiels($field)
        {
            ...
            $sqlSel="SELECT u_name, name, className, tel, Email, sex, weixinNum,type,image FROM $tableName WHERE className='" . $field . "'";
